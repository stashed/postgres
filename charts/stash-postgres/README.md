...
$ helm repo update
$ helm install stash-postgres-9.6.19-v5 appscode/stash-postgres -n kube-system --version=9.6.19-v5
```
...
To install the chart with the release name `stash-postgres-9.6.19-v5`:
...
To uninstall/delete the `stash-postgres-9.6.19-v5`:

```console
$ helm delete stash-postgres-9.6.19-v5 -n kube-system
```
...
| image.tag        | Tag of the image that is used to backup/restore PostgreSQL database. This is usually same as the database version it can backup. | `9.6.19-v5`      |
...
```console
$ helm install stash-postgres-9.6.19-v5 appscode/stash-postgres -n kube-system --version=9.6.19-v5 --set image.registry=stashed
```
...
```console
$ helm install stash-postgres-9.6.19-v5 appscode/stash-postgres -n kube-system --version=9.6.19-v5 --values values.yaml
```
