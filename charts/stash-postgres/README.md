...
$ helm repo update
$ helm install stash-postgres-v2020.09.16 appscode/stash-postgres -n kube-system --version=v2020.09.16
```
...
To install the chart with the release name `stash-postgres-v2020.09.16`:
...
To uninstall/delete the `stash-postgres-v2020.09.16`:

```console
$ helm delete stash-postgres-v2020.09.16 -n kube-system
```
...
| image.tag        | Tag of the image that is used to backup/restore PostgreSQL database. This is usually same as the database version it can backup. | `v2020.09.16`    |
...
```console
$ helm install stash-postgres-v2020.09.16 appscode/stash-postgres -n kube-system --version=v2020.09.16 --set image.registry=stashed
```
...
```console
$ helm install stash-postgres-v2020.09.16 appscode/stash-postgres -n kube-system --version=v2020.09.16 --values values.yaml
```
