...
$ helm repo update
$ helm install stash-postgres-12.4.0-v1 appscode/stash-postgres -n kube-system --version=12.4.0-v1
```
...
To install the chart with the release name `stash-postgres-12.4.0-v1`:
...
To uninstall/delete the `stash-postgres-12.4.0-v1`:

```console
$ helm delete stash-postgres-12.4.0-v1 -n kube-system
```
...
| image.tag        | Tag of the image that is used to backup/restore PostgreSQL database. This is usually same as the database version it can backup. | `12.4.0-v1`      |
...
```console
$ helm install stash-postgres-12.4.0-v1 appscode/stash-postgres -n kube-system --version=12.4.0-v1 --set image.registry=stashed
```
...
```console
$ helm install stash-postgres-12.4.0-v1 appscode/stash-postgres -n kube-system --version=12.4.0-v1 --values values.yaml
```
