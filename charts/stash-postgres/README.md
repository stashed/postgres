...
$ helm repo update
$ helm install stash-postgres-12.4.0-v2 appscode/stash-postgres -n kube-system --version=12.4.0-v2
```
...
To install the chart with the release name `stash-postgres-12.4.0-v2`:
...
To uninstall/delete the `stash-postgres-12.4.0-v2`:

```console
$ helm delete stash-postgres-12.4.0-v2 -n kube-system
```
...
| image.tag        | Tag of the image that is used to backup/restore PostgreSQL database. This is usually same as the database version it can backup. | `12.4.0-v2`      |
...
```console
$ helm install stash-postgres-12.4.0-v2 appscode/stash-postgres -n kube-system --version=12.4.0-v2 --set image.registry=stashed
```
...
```console
$ helm install stash-postgres-12.4.0-v2 appscode/stash-postgres -n kube-system --version=12.4.0-v2 --values values.yaml
```
