...
| `global.tag`             | Tag of the image that is used to backup/restore PostgreSQL database. This is usually same as the database version it can backup. | `11.1`           |
...
```console
helm install --name postgres-stash-11.1 --set global.metrics.enabled=false appscode/postgres-stash
```
